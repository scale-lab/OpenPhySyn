...
DatabaseSdcNetwork::DatabaseSdcNetwork(Network* network) : SdcNetwork(network)
{
}

// Override SdcNetwork to NetworkNameAdapter.
Instance*
DatabaseSdcNetwork::findInstance(const char* path_name) const
{
...
void
DatabaseSdcNetwork::findInstancesMatching(const Instance*,
...
void
DatabaseSdcNetwork::findInstancesMatching1(const PatternMatch* pattern,
                                           InstanceSeq*        insts) const
{
...
void
DatabaseSdcNetwork::findNetsMatching(const Instance*,
                                     const PatternMatch* pattern,
                                     NetSeq*             nets) const
{
    if (pattern->hasWildcards())
...
void
DatabaseSdcNetwork::findNetsMatching1(const PatternMatch* pattern,
                                      NetSeq*             nets) const
{
...
void
DatabaseSdcNetwork::findPinsMatching(const Instance*     instance,
                                     const PatternMatch* pattern,
                                     PinSeq*             pins) const
{
...
void
DatabaseSdcNetwork::findMatchingPins(const Instance*     instance,
    ...
    if (instance != network_->topInstance())
    {
        ...
        while (port_iter->hasNext())
        {
            ...
            if (network_->hasMembers(port))
            {
                ...
                PortMemberIterator* member_iter =
...
Pin*
DatabaseSdcNetwork::findPin(const char* path_name) const
{
    char *inst_path, *port_name;
